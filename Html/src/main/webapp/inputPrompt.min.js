...
(function (e, i, a, b) {
	...
	h.proto = {
		...
		query : function (l) {
			var k = e(".search_box form").attr("action");
			var m = isNewLang(k);
			k = m ? (domainURI(k) + m).replace(/\/$/, "") : domainURI(k);
			if (langt == "zh-tw") {
				i.location.href = k + "/big5/" + myEncodeURI(l)
			} else {
				i.location.href = k + "/" + myEncodeURI(l)
			}
		}
	};
	h.juhai = function () {
		this.query = function (k) {
			i.location.href = "/" + myEncodeURI(k)
		}
	};
	h.juhai.prototype = h.proto;
	h.hanyu = function () {
		this.query = function (k) {
			i.location.href = "/" + encodeURIComponent(k)
		}
	};
	h.abbr = function () {
		this.hasSugg = true;
		...
		this.getList = function (l, u) {
			...
			if (e.isArray(t) && t.length > 0) {
				...
				if (u) {
					for (var n = 0, m = t.length; n < m; n++) {
						var p = t[n].gt.replace(k, "<b>$1</b>");
						o += '<dl data="' + t[n].gt + '"><dt>' + p + "&nbsp;&nbsp;<span>" + t[n].g + "</span>&nbsp;&nbsp;<span>" + t[n].e + "</span></dt><dd></dd></dl>"
					}
				} else {
					for (var n = 0, m = t.length; n < m; n++) {
						var p = t[n].gt.replace(k, "<b>$1</b>");
						o += '<dl data="' + t[n].gt + '"><dt>' + p + "</dt><dd>" + t[n].e + "</dd></dl>"
					}
				}
				return o
			} else {
	...
	h.gdh.prototype = h.proto;
	h.shh = function () {
		this.query = function (k) {
			i.location.href = "/" + encodeURIComponent(k)
		}
	};
	h.shh.prototype = h.proto;
	var d = function () {
		var k = this;
	...
	function g(k) {
		if (g.instances[k]) {
			return g.instances[k]
		} else {
			if (h[k]) {
				return (g.instances[k] = new h[k]())
			} else {
				return false
			}
		}
	...
	d.prototype = {
		...
		init : function (x) {
			...
			var v = e('<a class="close" href="javascript:void(0)">关闭输入提示</a>').click(function () {
					r.disablePrompt()
				});
			r.containerHeight = u;
			r.listHeight = u;
	...
	e(a).bind("click", function (m) {
		if (f.length > 0) {
			for (var l = 0, k = f.length; l < k; l++) {
				if (e(m.target).parent().attr("id") != "sx-botton" && e(m.target).parent().attr("id") != "dict-draw-panel") {
					f[l].attemptHide(m.target);
					e("#dict-draw-panel").hide()
				}
			}
		}
		return
