...
var $user_id = getCookie("DictCN_uid"), $user_name = getCookie("DictCN_username"),
...
var navGoUrl = function (a, d) {
    var b = a.attr("target");
    if (b && (b == "_blank" || jtarget == "blank")) {
        window.open(d)
    } else {
        window.location.href = d
    }
};
var allowParam = function (a, b) {
    if (!b) {
        return false
    }
    if (("," + b + ",").indexOf("," + a + ",") != -1) {
...
!function (a) {
    a.click(function () {
        var i = $(this), f = $("#q"), h = i.attr("href"),
            g = (f.val() == (f.attr("placeholder") || "������Ӣ/���ִʾ�") ? "" : f.val()), d = $(this).attr("data-param"),
            b = "";
        if (crumb != undefined && crumb == "index" && typeof g_curreci != "undefined" && g_curreci == g) {
            return true
        }
        if (f.length && g.length) {
            if (/^[A-Za-z]+$/.test(g)) {
                b = allowParam("en", d) ? h.replace(/\/$/, "") + "/" + g : h;
                navGoUrl(i, b);
                return false
            } else {
                if (isAllChinese(g)) {
                    b = allowParam("zh", d) ? h.replace(/\/$/, "") + "/" + myEncodeURI(g) : h;
                    navGoUrl(i, b);
                    return false
                } else {
                    b = allowParam("other", d) ? h.replace(/\/$/, "") + "/" + myEncodeURI(g) : h;
                    navGoUrl(i, b);
                    return false
                }
            }
...
$(function () {
    ...
    $(function () {
        $(".tb1").click(function (f) {
            getCookie("dicthuaci") == "off" ? $(".popup01 input[name=huaci-opt]").eq(1).attr("checked", "checked") : $(".popup01 input[name=huaci-opt]").eq(0).attr("checked", "checked");
            $(".popup01 input[name=sound-opt]").eq(_dict_config.ss & 1).attr("checked", "checked");
            _dict_config.ss & 2 && $(".popup01 input[type=checkbox]").attr("checked", "checked");
            $(".tbcont_0").css("top", $(this).offset().top - ($(this).height() * 5) - 20).show()
        });
...
jQuery(function () {
    var a = '<em action="feedback" onclick="feedBackForm(this);" title="�������">�������</em><i>|</i><a title="ע��" href="http://passport.dict.cn/register">ע��</a><i>|</i><a title="��¼" href="http://passport.dict.cn/login">��¼</a>';
    var b = '<em action="feedback" onclick="feedBackForm(this);" title="�������">�������</em><i>|</i><div class="more"><a class="morehover"><span>' + getCookie("DictCN_username") + '</span></a><div class="personinfo"><ol><ul><li><a target="_blank" href="http://account.dict.cn/">����</a></li><li><a class="logout" href="http://passport.dict.cn/logout">�˳�</a></li></ul></ol></div></div>';
    if ($user_auth) {
...
}(getCookie("dicthe"), getCookie("dicthc"), getCookie("dicthh"));
$(function () {
    if (cur_dict == "hanyu") {
        var b = $(".search"), d = $("#q"),
...
function setCookie(a, d) {
    ...
    document.cookie = a + "=" + encodeURIComponent(d) + ";expires=" + f.toGMTString() + ";path=/;domain=.dict.cn"
}
...
$(function () {
    var b = "http://xuehai.cn/reg.gif?uvid=" + getCookie("uvid");
    var a = "<img style='display:none' src='" + b + "' />";
    $("body").append(a)
});
function stringToDateTime(j) {
    var b = 1000;
    var g = b * 60;
    var k = g * 60;
    var l = k * 24;
    ...
    if (m > a * 2) {
        ...
        if (m > a) {
            ...
            if (m > l * 7) {
                ...
                if (m > l) {
                    return (Math.floor(m / l) + "��ǰ")
                } else {
                    if (m > k) {
                        return (Math.floor(m / k) + "Сʱǰ")
                    } else {
                        if (m > g) {
                            return (Math.floor(m / g) + "����ǰ")
                        } else {
                            if (m > b) {
                                return (Math.floor(m / b) + "��ǰ")
                            } else {
                                return (m + " error ")
                            }
                        }
...
function saveConfig2Server() {
    var a = getCookie("DictCN_savecfg");
    if (a && _dict_config && _dict_config.rv > 0) {
        $.getJSON("http://www" + _dict_config.rv + ".dict.cn/ajax/user_config.php?act=save&" + a + "&jsoncallback=?", function (b) {
            if (b) {
                if (b.flag == "ok") {
...
function saveConfig() {
    ...
    var a, g = save = "", d, f;
    var b = getCookie("DictCN_cfg");
    var i = {};
    $.extend(i, _dict_config);
    while ((a = loadConfig.re.exec(b)) != null) {
        d = false;
        if ($.inArray(a[1], saveConfig.check) != -1) {
            if (_dict_config[a[1]] != parseInt(a[2])) {
                save += a[1] + "=" + _dict_config[a[1]] + "&";
                d = true
            }
        }
        if (d) {
            g += a[1] + _dict_config[a[1]]
        } else {
            g += a[1] + a[2]
        }
        delete i[a[1]]
    ...
    f = parseInt(getCookie("DictCN_time"));
    setCookie("DictCN_cfg", g, f);
...
function loadConfig(f) {
    saveConfig2Server();
    if (window._dict_config && window._dict_config.load) {
        return
    }
    var b = getCookie("DictCN_cfg");
    var d = {};
    ...
    if (b && b.length > 1) {
        ...
        if (f > 0) {
            $.getJSON("http://www" + f + ".dict.cn/ajax/user_config.php?act=load&jsoncallback=?", function (g) {
                if (g) {
                    if (g.flag == "ok") {
                        loadConfig()
                    } else {
                        if (g.flag == "no" && g.server > 0) {
                            loadConfig(status);
                            return
                        } else {
    ...
    if (window._dict_config) {
        $.extend(window._dict_config, d)
    } else {
        window._dict_config = d
    }
}
...
function setdictEnable(b) {
    var a = parseInt(getCookie("DictCN_time"));
    if (b) {
        b = false;
        setCookie("dicthuaci", "on", a);
        setCookie("dictstate", "1", a)
    } else {
        setCookie("dicthuaci", "off", a);
        setCookie("dictstate", "0", a)
    }
}
